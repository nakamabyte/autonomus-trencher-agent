...
export function formatOpenPosition(position, decision = {}) {
  ...
  const tx = position.entry_signature 
    ? `[${position.entry_signature.slice(0, 6)}...${position.entry_signature.slice(-4)}](https://solscan.io/tx/${position.entry_signature})` 
    : 'N/A';
  const token = position.mint 
    ? `[${position.mint.slice(0, 6)}...${position.mint.slice(-4)}](https://gmgn.ai/sol/token/${position.mint})` 
    : 'N/A';
