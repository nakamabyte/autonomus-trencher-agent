...
function x402(config) {
  return function (req, res, next) {
    ...
    if (!authHeader.startsWith('L402')) {
      const recipient = typeof config.recipient === 'function' ? config.recipient() : (config.recipient || config.paymentRecipient);
      ...
      res.status(402)
         ...
         .json({
           ...
           message: `Please pay ${config.price} ${config.currency} to ${recipient}`,
