    ...
    >
      <div
        ...
        style={{
          ...
          background: '#0c0c0c', border: `1px solid ${accentColor}22`,
      ...
      >
        ...
        <div style={{
          ...
          background: isWin
            ...
            : 'linear-gradient(135deg, rgba(248,113,113,0.05) 0%, transparent 60%)',
        }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
            <div>
              <div style={{
                fontSize: '10px', letterSpacing: '0.12em', opacity: 0.4,
                fontFamily: 'var(--fm)', marginBottom: '4px',
                textTransform: 'uppercase',
              }}>Autonomous Trencher Agent</div>
          ...
          <div style={{
            ...
            padding: '10px 20px', borderRadius: '6px', marginBottom: '20px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
            {[
              ['PNL', <span style={{ color: accentColor, fontWeight: 700 }}>{sign}{pos.pnl_percent.toFixed(2)}%</span>],
              ['Invested', <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><span style={{ color: '#9945FF', fontSize: '10px' }}>◎</span>{pos.size_sol.toFixed(3)}</span>],
              ['Position', <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><span style={{ color: '#9945FF', fontSize: '10px' }}>◎</span>{(pos.size_sol + pos.pnl_sol).toFixed(3)}</span>],
              ['Strategy', <span style={{ textTransform: 'uppercase', fontFamily: 'var(--fm)', fontSize: '11px' }}>{pos.strategy || '—'}</span>],
              ['Entry MCap', <span>{formatMcap(pos.entry_mcap)}</span>],
              ['Hold Time', <span>{holdStr}</span>],
              ['Exit', <span>{exitLabel}</span>],
            ].map(([label, val], i) => (
              <div key={i} style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '8px',
              }}>
                <span style={{ fontSize: '13px', opacity: 0.5 }}>{label as string}</span>
                <span style={{ fontSize: '13px', color: '#fff' }}>{val as React.ReactNode}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            <a href="https://trencher-agent.vercel.app" target="_blank" rel="noreferrer"
              style={{ fontSize: '11px', opacity: 0.4, color: '#fff', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>
...
export function PlatformHistory({ metrics, rightOffset = 16, logHeight }: PlatformHistoryProps) {
  ...
  return (
    <>
      ...
      {selectedPos && <PnlModal pos={selectedPos} onClose={() => setSelectedPos(null)} />}
    </>
  );
}
