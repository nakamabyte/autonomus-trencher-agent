...
Trencher Agent is a Telegram trench agent for screening noisy Pump-token flow with overlap signals, strategy gates, LLM selection, and dry-run/confirm/live execution.
...
1. Trencher Agent polls the signal server every `SIGNAL_POLL_MS`.
...
5. Trencher Agent routes approved buys through `dry_run`, `confirm`, or `live`.
6. Open positions are monitored every `POSITION_CHECK_MS` for TP, SL, trailing TP, max hold, and partial TP rules.

## Access

Trencher Agent requires a signal server URL and API key. The signal server aggregates fee-claim, graduated, and trending data from Pump.fun in real time — without it Trencher Agent has nothing to screen.
...
```bash
git clone git@github.com:YOUR_USERNAME/trencher-agent.git
cd trencher-agent
npm install
cp .env.example .env
```
...
```bash
pm2 start index.js --name trencher-agent
pm2 save
```
...
`TELEGRAM_CHAT_ID` is the chat or group ID where Trencher Agent sends alerts and accepts commands. Only messages from this chat are processed.
...
If `SOLANA_RPC_URL`/`SOLANA_WS_URL` are not set, Trencher Agent falls back to Helius mainnet URLs and requires:
...
GMGN enriches candidates with holder count, liquidity, fee data, and social links. Set `GMGN_ENABLED=false` to skip it — Trencher Agent falls back to Jupiter/server data and the status line shows `off`. GMGN has aggressive rate limits; keep `GMGN_REQUEST_DELAY_MS` at 2500+ ms.
...
`LIVE_MIN_SOL_RESERVE` is the minimum SOL kept in the wallet after any buy. Trencher Agent refuses to execute if the balance would fall below this.
...
Trencher Agent uses `trencher-agent.sqlite` as source of truth. It stores:
...
- **Jupiter**: `fetchJupiterAsset` and `fetchJupiterHolders` are called per candidate and per position refresh cycle. At high throughput, you may hit 429s — Trencher Agent backs off automatically and retries from cache.
