...
import { PUMP_PROGRAM, PUMP_AMM, DISC_DIST_FEES, getActiveWsUrl, rotateWsUrl } from '../config.js';
...
export function startWebsocket() {
  ...
  function connect() {
    ...
    ws.on('close', () => {
      ...
      rotateWsUrl();
      const nextUrl = getActiveWsUrl();
      console.log(`[ws] closed, reconnecting to ${nextUrl} in 5s`);
      setTimeout(connect, 5000);
    });
    ws.on('error', error => console.log(`[ws] error on ${wsUrl}: ${error.message}`));
  }
  connect();
}
